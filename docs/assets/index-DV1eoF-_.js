...
fn swapBuffer(@builtin(global_invocation_id) global_invocation_id : vec3u) {
  ...
  dataRead.particles[idx] = particle;
}`,P=4096,Qn=0,ei=4*4,ge=[Math.random()*100,Math.random()*100,1+Math.random(),1+Math.random()],De=[0,-10],oe=[16,12],E={simulate:!0,simulationStep:.001,smoothlen:.5,pressureStiffness:.57,restDensity:4,particleMass:.08,viscosity:4,wallStiffness:6e3,iteration:4,reset:()=>{at()}},ti=E.particleMass*4/(Math.PI*Math.pow(E.smoothlen,8)),ni=E.particleMass*-30/(Math.PI*Math.pow(E.smoothlen,5)),ii=E.particleMass*20/(3*Math.PI*Math.pow(E.smoothlen,5)),oi=1*4+3*4+4*4+1*4+1*4+1*4+1*4+1*4+1*4+1*4+1*4+1*4+1*4+2*4+1*4+1*4+2*4,X=3*4+1*4+4*4+3*4+1*4+3*4+1*4+1*4+1*4+2*4,ri=4*4*4+3*4+4+3*4+4+0,ai=2*4+2*4+1*4+3*4,ne=document.querySelector("canvas"),si=await navigator.gpu.requestAdapter(),_=await si.requestDevice(),Ke=ne.getContext("webgpu"),Ze=window.devicePixelRatio;ne.width=ne.clientWidth*Ze;ne.height=ne.clientHeight*Ze;const Qe=navigator.gpu.getPreferredCanvasFormat(),et=new Fn([oe[0]/2,oe[1]/2,10],[oe[0]/2,oe[1]/2,0]),li=et.updateMVPMatrix();Ke.configure({device:_,format:Qe,alphaMode:"premultiplied"});const J=new Ee;J.add(E,"simulate");J.add(E,"simulationStep",.001,.005,.001);J.add(E,"smoothlen",.1,1,.1);J.add(E,"pressureStiffness",.1,1,.1);J.add(E,"restDensity",1,10,1);J.add(E,"particleMass",.01,.2,.01);J.add(E,"viscosity",.1,10,.1);J.add(E,"wallStiffness",1e3,6e3,1e3);J.add(E,"iteration",1,20,1);J.add(E,"reset");J.onChange(()=>{rt()});const ve=new qn;ve.showPanel(0);document.body.appendChild(ve.dom);const ye=_.createBuffer({size:P*X,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC}),ci=_.createBuffer({size:P*X,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.STORAGE}),Q=_.createBuffer({size:P*X,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ}),tt=_.createBuffer({size:ri,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),Ce=_.createBuffer({size:6*2*4,usage:GPUBufferUsage.VERTEX,mappedAtCreation:!0}),nt=_.createBuffer({size:oi,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),we=_.createBuffer({size:ai,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),it=_.createRenderPipeline({layout:"auto",vertex:{module:_.createShaderModule({code:ee}),buffers:[{arrayStride:X,stepMode:"instance",attributes:[{shaderLocation:0,offset:Qn,format:"float32x3"},{shaderLocation:1,offset:ei,format:"float32x4"}]},{arrayStride:2*4,stepMode:"vertex",attributes:[{shaderLocation:2,offset:0,format:"float32x2"}]}]},fragment:{module:_.createShaderModule({code:ee}),targets:[{format:Qe,blend:{color:{srcFactor:"src-alpha",dstFactor:"one",operation:"add"},alpha:{srcFactor:"zero",dstFactor:"one",operation:"add"}}}]},primitive:{topology:"triangle-list"},depthStencil:{depthWriteEnabled:!1,depthCompare:"less",format:"depth24plus"}}),ui=_.createTexture({size:[ne.width,ne.height],format:"depth24plus",usage:GPUTextureUsage.RENDER_ATTACHMENT}),hi=_.createBindGroup({layout:it.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:tt}}]}),Ve={colorAttachments:[{view:void 0,clearValue:[0,0,0,1],loadOp:"clear",storeOp:"store"}],depthStencilAttachment:{view:ui.createView(),depthClearValue:1,depthLoadOp:"clear",depthStoreOp:"store"}},N=1,pi=[-N,-N,+N,-N,-N,+N,-N,+N,+N,-N,+N,+N];new Float32Array(Ce.getMappedRange()).set(pi);Ce.unmap();const ot=_.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}}]}),re=_.createPipelineLayout({bindGroupLayouts:[ot]}),Z=_.createBindGroup({layout:ot,entries:[{binding:0,resource:{buffer:nt}},{binding:1,resource:{buffer:ye,offset:0,size:P*X}},{binding:2,resource:{buffer:ci,offset:0,size:P*X}},{binding:3,resource:{buffer:we}}]}),fi=_.createComputePipeline({layout:re,compute:{module:_.createShaderModule({code:ee}),entryPoint:"densityCS"}}),di=_.createComputePipeline({layout:re,compute:{module:_.createShaderModule({code:ee}),entryPoint:"pressureCS"}}),gi=_.createComputePipeline({layout:re,compute:{module:_.createShaderModule({code:ee}),entryPoint:"simulate"}}),mi=_.createComputePipeline({layout:re,compute:{module:_.createShaderModule({code:ee}),entryPoint:"init"}}),vi=_.createComputePipeline({layout:re,compute:{module:_.createShaderModule({code:ee}),entryPoint:"forceCS"}}),me=_.createComputePipeline({layout:re,compute:{module:_.createShaderModule({code:ee}),entryPoint:"swapBuffer"}}),rt=()=>{_.queue.writeBuffer(nt,0,new Float32Array([E.simulate?E.simulationStep:0,0,0,0,ge[0],ge[1],ge[2],ge[3],E.smoothlen,ti,ni,ii,E.pressureStiffness,E.restDensity,E.particleMass,E.viscosity,E.wallStiffness,E.iteration,De[0],De[1],oe[0],oe[1],0]))};rt();_.queue.writeBuffer(we,0,new Float32Array([0,0,0]));let Se=!1;document.addEventListener("mousedown",function(t){Se=!0});document.addEventListener("mouseup",function(t){Se=!1,_.queue.writeBuffer(we,0,new Float32Array([0,0,0]))});document.addEventListener("mousemove",function(t){if(Se){const e=et.screenToWorld(t.clientX,t.clientY,10),n=1;_.queue.writeBuffer(we,0,new Float32Array([e[0],e[1],n]))}});const at=()=>{const t=_.createCommandEncoder(),e=t.beginComputePass();e.setPipeline(mi),e.setBindGroup(0,Z),e.dispatchWorkgroups(Math.ceil(P/64)),e.end(),_.queue.submit([t.finish()])};at();async function st(){ve.begin(),_.queue.writeBuffer(tt,0,new Float32Array(li));const t=Ke.getCurrentTexture();Ve.colorAttachments[0].view=t.createView();const e=_.createCommandEncoder();for(let n=0;n<E.iteration;n++){{const i=e.beginComputePass();i.setPipeline(fi),i.setBindGroup(0,Z),i.dispatchWorkgroups(Math.ceil(P/64)),i.end()}{const i=e.beginComputePass();i.setPipeline(me),i.setBindGroup(0,Z),i.dispatchWorkgroups(Math.ceil(P/64)),i.end()}{const i=e.beginComputePass();i.setPipeline(di),i.setBindGroup(0,Z),i.dispatchWorkgroups(Math.ceil(P/64)),i.end()}{const i=e.beginComputePass();i.setPipeline(me),i.setBindGroup(0,Z),i.dispatchWorkgroups(Math.ceil(P/64)),i.end()}{const i=e.beginComputePass();i.setPipeline(vi),i.setBindGroup(0,Z),i.dispatchWorkgroups(Math.ceil(P/64)),i.end()}{const i=e.beginComputePass();i.setPipeline(me),i.setBindGroup(0,Z),i.dispatchWorkgroups(Math.ceil(P/64)),i.end()}{const i=e.beginComputePass();i.setPipeline(gi),i.setBindGroup(0,Z),i.dispatchWorkgroups(Math.ceil(P/64)),i.end()}{const i=e.beginComputePass();i.setPipeline(me),i.setBindGroup(0,Z),i.dispatchWorkgroups(Math.ceil(P/64)),i.end()}}{const n=e.beginRenderPass(Ve);n.setPipeline(it),n.setBindGroup(0,hi),n.setVertexBuffer(0,ye),n.setVertexBuffer(1,Ce),n.draw(6,P,0,0),n.end()}_.queue.submit([e.finish()]),ve.end(),requestAnimationFrame(st)}requestAnimationFrame(st);const lt=t=>{const e=[],n=new Float32Array(t);for(let i=0;i<P;i++){const o=i*X/4,r={position:[n[o],n[o+1],n[o+2]],color:[n[o+4],n[o+5],n[o+6],n[o+7]],velocity:[n[o+8],n[o+9],n[o+10]],acceleration:[n[o+12],n[o+13],n[o+14]],density:n[o+15],pressure:n[o+16]};e.push(r)}return e};async function yi(){const t=_.createCommandEncoder();t.copyBufferToBuffer(ye,0,Q,0,P*X);const e=t.finish();_.queue.submit([e]);try{await Q.mapAsync(GPUMapMode.READ,0,P*X);const i=Q.getMappedRange(0,P*X).slice();Q.unmap();const o=lt(i);console.log(o)}catch(n){console.error("Error mapping buffer:",n)}}async function wi(){const t=_.createCommandEncoder();t.copyBufferToBuffer(ye,0,Q,0,P*X);const e=t.finish();_.queue.submit([e]);try{await Q.mapAsync(GPUMapMode.READ,0,P*X);const i=Q.getMappedRange(0,P*X).slice();Q.unmap();const o=lt(i);console.log("density : ",o[0].density,"pressure : ",o[0].pressure,"position : ",o[0].position)}catch(n){console.error("Error mapping buffer:",n)}}document.addEventListener("keydown",t=>{(t.key==="f"||t.key==="F")&&setInterval(wi,100)});document.addEventListener("keydown",t=>{(t.key==="c"||t.key==="C")&&yi()});
